...
城市节点作品保存地点正文、地图专用资料和相对定位：

- `detail`：完整地点详情，原文写入 `玩家地点 - 名称` 世界书正文，长度上限 100000 字符。
- `mapIntro`：地图详情面板使用的简短介绍，不写入世界书正文。
- `mapNotes`：地图备注列表，不写入世界书正文。
- `features`：地图功能开关，不写入世界书正文。
- `plate`
- `[0, 1]` 范围内的 `localPos`
- 静态 `anchorId`
- `anchorName`
...
从创意工坊安装城市节点会免费生成新的本地节点 ID，并写入现有 `系统配置.地图.自建节点` 结构；不会扣除玩家金钱。旧作品包中的 `intro`、`draw`、`special` 会分别兼容转换为 `detail`、`mapIntro`、`mapNotes`。
